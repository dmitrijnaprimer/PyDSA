...
#
# Modified for Python 3.x compatibility - SP-ST, OCT 2025

import math
import time
import numpy
import tkinter.font as tkFont # Changed import
import sys
import pyvisa as visa # Changed import
from time import sleep
from tkinter import * # Changed import
from tkinter.filedialog import askopenfilename # Changed import
from tkinter.simpledialog import askstring # Changed import
from tkinter.messagebox import * # Changed import
...
def Bnot():
    print("Routine not made yet") # Changed print statement
...
# Get the USB device, e.g. 'USB0::0x1AB1::0x0588::DS1ED141904883'
                rm = visa.ResourceManager() # Initialize ResourceManager for PyVISA
                print(f"PyVISA Backend: {rm.list_resources()}") # Debug: Print available resources and backend
                instruments = rm.list_resources() # Use ResourceManager.list_resources()
                # Filter for Rigol DS1xxx series based on the output you provided
                usb = [x for x in instruments if 'USB' in x and '0x1AB1' in x and '0x0588' in x]
                # usb = [x for x in instruments if 'USB' in x] # Original filter, less specific
                if len(usb) != 1:
                    print('Bad instrument list or no Rigol DS1xxx found', instruments) # Changed print statement
                    sys.exit(-1)
                print(f"Found Rigol device: {usb[0]}") # Debug: Print found device
                scope = rm.open_resource(usb[0]) # Use ResourceManager.open_resource()
                # Set timeout and chunk size after opening the resource
                scope.timeout = 20000 # Set timeout in milliseconds (PyVISA style)
                scope.chunk_size = 1024000 # Set chunk size if needed (check PyVISA docs)
                print("Scope opened successfully, IDN:", scope.query("*IDN?").strip()) # Debug: Check connection and get ID
                RUNstatus = 2
            except Exception as e: # Catch exception more generally
                RUNstatus = 0
                print(f"VISA Error: Cannot open scope - {e}") # Changed print statement
                showerror("VISA Error","Cannot open scope")
# get metadata
            #sample_rate = float(scope.query(':ACQ:SAMP?'))
...
# Set the scope the way we want it
            if SAMPLEdepth == 0:
                scope.write(':ACQ:MEMD NORM') # Normal memory type
            else:
                scope.write(':ACQ:MEMD LONG') # Long memory type

            # --- Trigger Configuration ---
            scope.write(':TRIG:MODE EDGE')        # Ensure trigger mode is Edge
            scope.write(':TRIG:EDGE:SOUR CHAN1')  # Set trigger source to Channel 1
            scope.write(':TRIG:EDGE:SLOP POS')    # Set trigger slope (POS or NEG, adjust if needed)
            scope.write(':TRIG:EDGE:LEV 0.0')     # Set trigger level (adjust if needed, 0V as example)
            scope.write(':TRIG:EDGE:COUP DC')     # Set trigger coupling (DC, AC, adjust if needed)
            scope.write(':TRIG:EDGE:SWE SING')    # Ensure single sweep mode
            # --- End Trigger Configuration ---

            scope.write(":RUN") # Start acquisition
            print("RUN command sent, checking trigger status...") # Debug: Indicate RUN sent
            sleep(0.1) # Small delay after RUN command (might help scope state machine)
            #txt = "Trig"
            #x = X0L + 250
            #y = Y0T+GRH+32
            #IDtxt  = ca.create_text (x, y, text=txt, anchor=W, fill=COLORyellow)
            #root.update()       # update screen
            trig_stat = scope.query(':TRIG:STAT?').strip() # Query trigger status
            print(f"Initial trigger status after RUN: {trig_stat}") # Debug: Check trigger status immediately after RUN
            while trig_stat != 'STOP':
                sleep(0.1)
                trig_stat = scope.query(':TRIG:STAT?').strip() # Query trigger status again
                print(f"Waiting... Trigger status: {trig_stat}") # Debug: Check trigger status
            print("Trigger status is STOP, proceeding with data acquisition.") # Debug: Indicate exit from loop
            #sleep(0.1)
    # Grab the raw data from channel 1, which will take a few seconds for long buffer mode
            scope.write(":STOP") # Ensure scope is stopped after acquisition
            ...
            root.update()       # update screen 
            print("Querying waveform data...") # Debug: Indicate data fetch
            signals_raw = scope.query_binary_values(":WAV:DATA? CHAN1", datatype='B', is_big_endian=False, container=bytes) # Use query_binary_values for raw data
            print(f"Received {len(signals_raw)} raw bytes") # Debug: Check data size
            SAMPLErate = float(scope.query(':ACQ:SAMP?')) # Use query for string response, then convert
            print(f'Data size: {len(signals_raw)}, Sample rate: {SAMPLErate}') # Changed print statement
            sleep(0.1)               
# convert data from (inverted) bytes to an array of scaled floats
# this magic from Matthew Mets
            SIGNAL1 = numpy.frombuffer(signals_raw, dtype='B') # Use the bytes object from query_binary_values
            #print(SIGNAL1)
            SIGNAL1 = (SIGNAL1 * -1 + 255) -130  # invert
            #print(SIGNAL1)
            SIGNAL1 = SIGNAL1/127.0 # scale 10 +-1, has a slight DC offset
            #print(SIGNAL1)
            print(f"Final SIGNAL1 length: {len(SIGNAL1)}, max: {numpy.max(SIGNAL1)}, min: {numpy.min(SIGNAL1)}") # Debug: Check processed signal
        ...
        if (RUNstatus == 3) or (RUNstatus == 4):
            scope.write(":KEY:FORCE") # Ensure scope is ready for next trigger (or reset state)
            scope.close() # Close the resource
            if RUNstatus == 3:
...
# FFT can take a long time!
    ...
    if len(SIGNAL1) >= 1048576: # ensure only valid buffer sizes
        ...
        print(f"Skipping FFT: SIGNAL1 length {len(SIGNAL1)} is too small (< 8192).") # Debug: Skip FFT if signal too short
